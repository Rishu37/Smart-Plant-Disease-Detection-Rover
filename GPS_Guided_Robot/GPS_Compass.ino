void calibrateCompass()                                           // Experimental Use Only to Calibrate Magnetometer/ Compass
{
  ...
  if (compass.getX() < minX) minX = compass.getX();
  if (compass.getX() > maxX) maxX = compass.getX();
  if (compass.getY() < minY) minY = compass.getY();
  if (compass.getY() > maxY) maxY = compass.getY();
